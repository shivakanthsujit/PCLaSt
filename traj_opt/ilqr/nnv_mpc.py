...
from ilqr_mpc.mpc.mpc import QuadCost, LinDx
...
class NNV_MPC(Module):
    ...
    def __init__(
        ...
        # gamma is the list of trust region scaling multipliers
        self.gamma = gamma
        self.tr_num = gamma.size(0)
    ...
    def forward(self, x_init, cost, dx):
        ...
        for i in range(self.lqr_iter):
            ...
            xinit = x_init.repeat((tr_num, 1))
            u = u.repeat((1, tr_num, 1))
            u = Variable(util.detach_maybe(u), requires_grad=True)
            # Linearize the dynamics around the current trajectory.
            x = util.get_traj(self.T, u, x_init=xinit, dynamics=dx)

            F, f = self.linearize_dynamics(x, util.detach_maybe(u), dx, gamma = gamma)
            # TODO: need to double check if the original order of batch-trust region number is recovered
            F = F.view(self.T-1, n_batch*tr_num, self.n_state, self.n_state + self.n_ctrl)
            f = f.view(self.T-1, n_batch*tr_num, self.n_state)

            if isinstance(cost, QuadCost):
                C, c = cost.C, cost.c
                C, c = C.repeat((1, tr_num, 1, 1)), c.repeat((1, tr_num, 1))
                cost_aug = QuadCost(C, c)
            else:
            ...
            x, u, n_total_qp_iter, costs, full_du_norm, mean_alphas = \
              self.solve_lqr_subproblem(xinit, C, c, F, f, cost_aug, dx, x, u)

            # select the winning trajectory
            temp = costs.view(tr_num, n_batch)
            # minimizing trust region sizes for each initial condition
            ind = torch.argmin(temp, dim = 0)

            mask = torch.Tensor.long(torch.zeros((tr_num, n_batch)))
            mask[ind, torch.arange(n_batch)] = 1
            ind_extract = mask.view(-1) > 0

            u = u[:, ind_extract, :]
            x = x[:, ind_extract, :]
            costs = costs[ind_extract]
            full_du_norm = full_du_norm[ind_extract]
        ...
        # TODO：find out why this final iteration is needed.
        xinit = x_init.repeat((tr_num, 1))
        u = u.repeat((1, tr_num, 1))
        u = Variable(util.detach_maybe(u), requires_grad=True)
        # Linearize the dynamics around the current trajectory.
        ...
        x, u = self.solve_lqr_subproblem(
            x_init, C, c, F, f, cost_aug, dx, x, u, no_op_forward=True)
...
""")
    ...
    # @profile
    def linearize_dynamics(self, x, u, dynamics, gamma):
        ...
        scaling_gamma = gamma.unsqueeze(0).repeat((n_batch, 1)).T.reshape(-1,1).unsqueeze(0).repeat((self.T-1, 1, 1))
        scaling_gamma = scaling_gamma.contiguous().view(-1, 1)

        F, f = dynamics.grad_input(_x, _u, gamma = scaling_gamma)
