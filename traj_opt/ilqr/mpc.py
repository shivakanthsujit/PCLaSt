...
class MPC(Module):
    ...
    def forward(self, x_init, cost, dx):
        ...
        # simulate the initial nominal trajectory
        u_init = u
        x_traj_init = util.get_traj(self.T, u_init, x_init=x_init, dynamics=dx)
        cost_init = util.get_cost(self.T, u_init, cost, dynamics = dx, x_init = x_init)

        temp_record = {
                    'x': list(torch.split(x_traj_init, split_size_or_sections=1, dim=1)),
                    'u': list(torch.split(u_init, split_size_or_sections=1, dim=1)),
                    'costs': cost_init,
                    'full_du_norm': None,
                    'run_time': 0.0,
                }
        ...
        ilqr_traj_record.append(temp_record)
        ...
        costs = best['costs']
        return (x, u, costs, ilqr_traj_record)
